...
def atom_pairs_from_one_residue(mnames: Union[str, List[str]],
                                atom_names_1: Union[str, List[str]],
                                atom_names_2: Union[str, List[str]],
                                residue_of_interest: List[int]):
    """

    :param mnames:
    :param atom_names_1:
    :param atom_names_2:
    :param residue_of_interest:
    :return:
    """
    ...
    def selector(frame: Frame):
        ...
        return [
            ...
            for first_atom in
            r.atoms.filter((mName == mname) & rId.is_in(*residue_of_interest) & aName.is_in(*atom_names_1))
            for second_atom in
            r.atoms.filter((mName == mname) & rId.is_in(*residue_of_interest) & aName.is_in(*atom_names_2))
        ]
